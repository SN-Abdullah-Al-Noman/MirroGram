...
from os import remove as osremove, rename as osrename, path as ospath, getcwd
...
async def edit_video_titles(user_id, file_path):
    ...
    new_file = ospath.join(directory, f"new_temp_{file_name}")
    command_probe_audio = ["ffprobe", "-v", "error", "-show_entries", "stream=index", "-select_streams", "a", "-of", "default=noprint_wrappers=1:nokey=1", file_path]
    try:
        probe_result_audio = scheck_output(command_probe_audio).decode("utf-8").strip()
        audio_stream_count = len(refindall(r'\d+', probe_result_audio))
    except:
    ...
    temp_file = ospath.join(directory, f"temp_{file_name}")
    cmd_remove_png = ["ffmpeg", "-i", file_path, "-map", "0", "-map", "-0:3", "-c", "copy", temp_file]
    srun(cmd_remove_png, check=True)
    osremove(file_path)

    cmd = ["ffmpeg", "-i", temp_file, "-map", "0", "-c", "copy"]
    ...
    srun(cmd, check=True)
    osremove(temp_file)
    osrename(new_file, ospath.join(directory, file_name))
